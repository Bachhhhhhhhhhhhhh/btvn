# -*- coding: utf-8 -*-
"""Extract LOG depreciation Excel → data dict / data.json."""
...
def find_xlsx(extra: Path | None = None) -> Path | None:
    if extra and Path(extra).exists():
        return Path(extra)
    for p in CANDIDATES:
        if p.exists():
            return p
    # any recent Bản Log in downloads
    for p in sorted(DOWNLOADS.glob("*Log*.xlsx"), key=lambda x: x.stat().st_mtime, reverse=True):
        if p.name.startswith("~$"):
            continue
        if "Log" in p.name or "log" in p.name:
            return p
    return None


def extract_from_path(xlsx_path: str | Path, as_of: datetime | None = None) -> dict:
    ...
    for r in budget_rows:
        gap = r["gaps"].get(
            "104Ki", (r["values"].get("104Ki", 0) - r["values"].get("103Ki", 0))
        )
        movers.append(
            {
                "key": f"{r['ccName']} · {r['glName']}",
    ...
    insights = [
        f"Kỳ cao nhất là {peak} với {(grand.get(peak) or totals_by_period[peak]):,.0f} VND.",
    ]
    if movers_up:
        insights.append(
    ...
    insights.append(
        f"Đang theo dõi {len(assets)} dòng tài sản · nguyên giá ~{sum(a['cost'] for a in assets)/1e9:.1f}B VND."
    )
    ...
    return {
        "meta": {
            ...
            "version": "4.0-live",
            "fileMtime": mtime,
            "fileMtimeIso": datetime.fromtimestamp(mtime).strftime("%Y-%m-%d %H:%M:%S"),
            "live": True,
        },
        "kpi": {
...
def extract_and_save(xlsx_path: str | Path | None = None, out_json: Path | None = None) -> dict:
    path = Path(xlsx_path) if xlsx_path else find_xlsx()
    if not path or not path.exists():
        raise FileNotFoundError("Excel source not found")
    data = extract_from_path(path)
    out = out_json or Path(__file__).resolve().parent / "data.json"
...
if __name__ == "__main__":
    d = extract_and_save()
    print("OK", d["meta"]["source"], "assets", d["kpi"]["assetCount"], "mtime", d["meta"]["fileMtimeIso"])
